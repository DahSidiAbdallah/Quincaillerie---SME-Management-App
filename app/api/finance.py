...
def get_finance_charts():
    """Get financial chart data for dashboard visualization"""
    auth_check = require_auth()
    if auth_check:
        return auth_check
    ...
    try:
        ...
        if period == 'week':
            ...
            expense_group_by = "DATE(expense_date)"
        elif period == 'month':
            ...
            expense_group_by = "strftime('%Y-%m', expense_date)"
        else:
            date_filter = "DATE('now', '-7 days')"
            group_by = "DATE(sale_date)"
            expense_group_by = "DATE(expense_date)"
        ...
        # Get expense data
        cursor.execute(f'''
            SELECT {expense_group_by} as period,
                   COALESCE(SUM(amount), 0) as expense
            FROM expenses
            WHERE DATE(expense_date) >= {date_filter}
            AND (is_deleted IS NULL OR is_deleted = 0)
            GROUP BY {expense_group_by}
            ORDER BY period
        ''')
        expense_data = [dict(row) for row in cursor.fetchall()]
        ...
        for item in revenue_data:
            ...
            revenue_dict[item['period']] = item['revenue']
        
        for item in expense_data:
            periods.add(item['period'])
            expense_dict[item['period']] = item['expense']
        ...
        chart_data = {
            ...
            'revenue': [revenue_dict.get(period, 0) for period in sorted_periods],
            'expense': [expense_dict.get(period, 0) for period in sorted_periods],
            'profit': [revenue_dict.get(period, 0) - expense_dict.get(period, 0) for period in sorted_periods]
        }
        ...
        return jsonify({
            ...
            'summary': {
                ...
                'net_profit': revenue_stats['total_revenue'] - expense_stats['total_expenses'],
                'sales_count': revenue_stats['sales_count'],
        ...
        return jsonify({'success': False, 'message': 'Erreur lors de la récupération des graphiques financiers'}), 500
