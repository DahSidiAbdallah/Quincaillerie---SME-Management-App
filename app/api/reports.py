...
def get_profit_analysis():
    """Get detailed profit analysis report"""
    auth_check = require_auth()
    if auth_check:
        return auth_check
    ...
    try:
        ...
        cursor.execute('''
            SELECT p.name, p.category,
                   ...
                   SUM(si.total_price) as revenue,
                   SUM(si.profit_margin) as profit,
                   AVG(si.profit_margin / si.quantity) as avg_profit_per_unit
            FROM sale_items si
            JOIN products p ON si.product_id = p.id
            JOIN sales s ON si.sale_id = s.id
            WHERE DATE(s.sale_date) BETWEEN ? AND ?
        ...
        profit_by_category = [dict(row) for row in cursor.fetchall()]
        
        # Daily profit trend
        cursor.execute('''
            SELECT DATE(s.sale_date) as date,
                   SUM(si.profit_margin) as daily_profit,
        ...
        daily_profit_trend = [dict(row) for row in cursor.fetchall()]
        
        # Calculate profit margins
        for item in profit_by_product:
            if item['revenue'] > 0:
                item['profit_margin_percent'] = (item['profit'] / item['revenue']) * 100
            else:
                item['profit_margin_percent'] = 0
        
        for item in profit_by_category:
            if item['total_revenue'] > 0:
                item['profit_margin_percent'] = (item['total_profit'] / item['total_revenue']) * 100
            else:
                item['profit_margin_percent'] = 0
        ...
        logger.error(f"Error generating profit analysis: {e}")
        return jsonify({'success': False, 'message': 'Erreur lors de l\'analyse des profits'}), 500
    ...
    try:
        ...
        cursor.execute('''
            SELECT s.customer_name,
                   ...
                   MAX(DATE(s.sale_date)) as last_purchase_date,
                   SUM(CASE WHEN s.is_credit = 1 THEN 1 ELSE 0 END) as credit_purchases
            FROM sales s
            WHERE s.customer_name != '' AND s.customer_name IS NOT NULL
            AND (s.is_deleted IS NULL OR s.is_deleted = 0)
        ...
        cursor.execute('''
            SELECT 
                CASE 
                    WHEN purchase_count >= 10 THEN 'Frequent (10+)'
                    WHEN purchase_count >= 5 THEN 'Regular (5-9)'
                    WHEN purchase_count >= 2 THEN 'Occasional (2-4)'
                    ELSE 'One-time (1)'
        ...
        frequency_analysis = [dict(row) for row in cursor.fetchall()]
        
        # Credit vs cash customers
        cursor.execute('''
            SELECT 
                CASE WHEN credit_ratio > 0.5 THEN 'Credit-heavy'
                     WHEN credit_ratio > 0.2 THEN 'Mixed'
                     ELSE 'Cash-only'
            ...
            FROM (
                SELECT customer_name,
                       SUM(total_amount) as total_spent,
                       SUM(CASE WHEN is_credit = 1 THEN total_amount ELSE 0 END) / SUM(total_amount) as credit_ratio
                FROM sales
                WHERE customer_name != '' AND customer_name IS NOT NULL
                AND (is_deleted IS NULL OR is_deleted = 0)
                GROUP BY customer_name
        ...
        payment_analysis = [dict(row) for row in cursor.fetchall()]
        ...
        return jsonify({'success': False, 'message': 'Erreur lors de l\'analyse des clients'}), 500
